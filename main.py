...
with tab_home:
    ...
    with col2:
        st.markdown(
            '<a href="https://kitchenmate.streamlit.app" target="_blank">'
            '<button style="background:#ff6b35;color:#0a0a0f;font-family:Sora,sans-serif;'
            'font-weight:700;border:none;border-radius:10px;padding:10px 24px;'
            'cursor:pointer;font-size:0.9rem;">🍳 Try KitchenMate</button></a>',
            unsafe_allow_html=True,
        )
    ...
    stats = [("2,841", "KitchenMate Users"), ("14,320", "Recipes Generated"), ("1", "Products Live"), ("1", "Coming Soon")]
    ...
    t1, _ = st.columns([2, 2])
    teasers = [
        ("🧠", "SankalpRoom", "AI-powered team collaboration — from brainstorm to delivery."),
    ]
    for col, (icon, name, desc) in zip([t1], teasers):
        with col:
            st.markdown(f"""
            <div class="teaser">
...
with tab_products:
    ...
    with c2:
        ...
        st.markdown(
            ...
            '<button style="background:#ff6b35;color:#0a0a0f;font-family:Sora,sans-serif;'
            'font-weight:700;border:none;border-radius:10px;padding:10px 24px;'
            'cursor:pointer;font-size:0.9rem;width:100%;">🚀 Try KitchenMate Now</button></a>',
            unsafe_allow_html=True,
        )
        st.markdown("""
