...
# CONFIG
# ─────────────────────────────────────────────
ADMIN_PASSWORD = "iis2024"  # ← change this
...
with tab_roadmap:
    ...
    for i, item in enumerate(st.session_state.roadmap):
        ...
        # Escape all user-supplied text before embedding in HTML
        ...
        safe_emoji  = item.get("emoji", "")   # emoji don't need escaping
        ...
        with col_card:
            ...
            <div class="card" style="margin-bottom:4px;">
                <div style="display:flex;align-items:flex-start;gap:14px;">
                    <div style="font-size:1.8rem;">{safe_emoji}</div>
                    <div style="flex:1;">
                        <div style="display:flex;align-items:center;gap:10px;margin-bottom:6px;">
                            {pulse}
                            <span style="font-family:Sora,sans-serif;font-size:1.05rem;font-weight:700;color:{T['heading']};">{safe_title}</span>
                            <span class="badge {badge_cls}">{safe_status}</span>
                        </div>
                        <p style="color:{T['text2']};font-size:0.88rem;line-height:1.6;margin:0;">{safe_desc}</p>
                        <div style="margin-top:10px;font-family:Sora,sans-serif;font-size:1.1rem;font-weight:800;color:{T['accent']};">▲ {votes} votes</div>
                    </div>
                </div>
            </div>""", unsafe_allow_html=True)
        with col_vote:
            st.markdown("<div style='height:32px;'></div>", unsafe_allow_html=True)
            # Use index (i) in key — guarantees uniqueness even if titles are duplicated
            if already_voted:
